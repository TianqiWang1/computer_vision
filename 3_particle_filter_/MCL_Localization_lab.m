...
%
    ...
    noise = normrnd(0,simpar.odometry_stdev,[1,length(priorbelief_particles)]);
    predict_particles = mod(priorbelief_particles + noise + uk, simpar.domain);
...
%
    ...
    % wk = P( zk | xk) 
     ...
     particle_weights = pdf('Normal', predict_particles, simpar.door_locations(1), simpar.door_stdev) + ...
...
%
    ...
    % random sample by weight
    updated_belief_particles = predict_particles(sum(bsxfun(@ge,rand(simpar.numberOfParticles,1), cumsum(particle_weights./sum(particle_weights))), 2) + 1 );
